...
class Index extends AdminBase
{
    ...
    public function json(int $do = 0)
    {
        ...
        $rids = implode(',', array_filter(explode(',', trim(strval($this->manUser['roleids']), ',')), 'strlen'));
        $roles = $rids ? \app\model\system\SystemRoles::where('state',1)->where('roleid','in',$rids)->column("role_name","roleid") : [];
        $rs =[
            'menus' => $cat ? ['cat'=>$cat,'menus'=>$data['menus']]: $data['menus'],
            'user'  => $this->manUser + ['roles' => $data['roles']] + ['rolem' => $roles] + ['dict' => Dict::cache((int)$do)]
        ];
