...
abstract class AdminBase extends BaseController
{
    ...
    /**
     * 权限判断
     * 简化角色：userid=1 超级管理员 / roleid=2 管理员（全权）/ 其他按角色菜单权限
     */
    private function isPower()
    {
        //后台外壳页(index/index)所有已登录用户均可加载(菜单按角色渲染)
        if(in_array($this->routeUri, ['index/index','index'])){
            if($this->manUser['userid'] > 1 && intval($this->manUser['roleid']) !== 2) return;
        }
        //系统首页(面板)仅管理员可用，注册用户跳转个人概览
        if($this->manUser['userid'] > 1 && intval($this->manUser['roleid']) !== 2 && $this->routeUri == 'index/main'){
            $this->redirect($this->appMap.'/my.index/index');
        }
        //菜单加载接口 index/json 所有已登录用户均可访问（仅返回自己拥有的菜单与个人信息）
        if($this->routeUri == 'index/json') return;
        //清理缓存/禁用IP 仅超级管理员(id=1)可用，其他角色一律无权
        if($this->manUser['userid'] > 1 && in_array($this->routeUri, ['index/clear','index/ip'])){
            $this->exitMsg('抱歉，您没有该项权限请联系管理员！',$this->request->isAjax() ? 401 : 400);
        }
        if($this->manUser['userid'] > 1 && intval($this->manUser['roleid']) !== 2){
            //注册用户仅校验后台路由；个人中心等通用功能由角色菜单控制
            $actions = $this->manUser['actions'] ?? [];
            if(!in_array($this->routeUri, $actions)){
                $this->exitMsg('抱歉，您没有该项权限请联系管理员！',$this->request->isAjax() ? 401 : 400);
            }
        }
    }
